export const FRONTEND_READY_EVENT_NAME = 'starter-tauri-app:frontend-ready'
export const STARTUP_RELOAD_ATTEMPT_STORAGE_NAME = 'starter-tauri-app:startup-reload-attempted'

declare global {
  interface Window {
    __starterTauriAppFrontendReady?: boolean
  }
}
...
export function markFrontendReady(options: FrontendReadyOptions = {}): void {
  ...
  win.__starterTauriAppFrontendReady = true
...
export function reloadFrontendOnceIfStartupFailed(
  ...
  if (win.__starterTauriAppFrontendReady === true) return false
