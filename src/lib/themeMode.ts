export const THEME_MODE_STORAGE_KEY = 'starter-tauri-app-theme-mode'
...
function readStarterSettingsThemeMode(storage: ThemeStorage): ThemeMode | null {
  try {
    const rawSettings = storage.getItem(STARTER_SETTINGS_STORAGE_KEY)
    if (!rawSettings) return null
    const settings = JSON.parse(rawSettings) as { themeMode?: unknown }
...
export function readStoredThemeMode(storage: ThemeStorage): ThemeMode | null {
  ...
  return readStarterSettingsThemeMode(storage)
}
