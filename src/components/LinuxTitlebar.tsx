...
import { createTranslator, resolveEffectiveLocale, type AppLocale } from '../lib/i18n'
...
export function LinuxTitlebar({ locale: localeOverride }: LinuxTitlebarProps = {}) {
  ...
  return (
    <>
      ...
      <div
        ref={dragRegionRef}
        className="fixed top-0 right-0 left-0 z-[1000] flex items-center justify-between border-b border-border bg-background px-3 select-none"
        style={{ height: LINUX_TITLEBAR_HEIGHT }}
        data-testid="linux-titlebar"
      >
        <div className="flex h-full items-center text-[13px] font-medium text-foreground/80">
          {t('app.name')}
        </div>
...
function readDocumentLocale(): AppLocale {
  ...
  return resolveEffectiveLocale(null, [document.documentElement.lang])
}
...
function TitlebarWindowControls({
  ...
  const resizeLabel = t(maximized ? 'window.restore' : 'window.maximize')
