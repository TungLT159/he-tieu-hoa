...
describe('LinuxTitlebar', () => {
  ...
  it('localizes titlebar window control labels', () => {
    render(<LinuxTitlebar locale="vi" />)

    expect(screen.getByRole('button', { name: 'Thu nhỏ' })).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Phóng to' })).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Đóng cửa sổ' })).toBeTruthy()
  })
})
